...
remove_project_runtime() {
  ...
  killall -9 Xvfb pulseaudio openbox sunshine chromium chromium-browser google-chrome-stable chrome firefox 2>/dev/null || true
  pkill -9 -x Xvfb || true
...
if [[ "$UNINSTALL_DEPS" = true ]]; then
  ...
  apt-get remove --purge -y xvfb openbox chromium chromium-browser google-chrome-stable firefox jq pulseaudio dbus-x11 || true
